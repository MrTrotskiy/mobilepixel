...
exports.registerCoreTools = void 0;
const zod_1 = require("zod");
const node_fs_1 = __importDefault(require("node:fs"));
const android_1 = require("../../platforms/android");
const ios_1 = require("../../platforms/ios");
const ios_2 = require("../../platforms/ios");
const android_2 = require("../../platforms/android");
 ...
 */
const registerCoreTools = context => {
    const { tool, getRobotFromDevice, simulatorManager, noParams } = context;
    // ===========================
    // DEVICE DISCOVERY
    // ===========================
    tool("mobile_list_available_devices", "List all available devices. This includes both physical devices and simulators. If there is more than one device returned, you need to let the user select one of them.", { noParams }, async ({}) => {
        const iosManager = new ios_2.IosManager();
        const androidManager = new android_2.AndroidDeviceManager();
    ...
    // BASIC INTERACTIONS
    // ===========================
    tool("mobile_click_on_screen_at_coordinates", `LOW-LEVEL TOOL: Click at exact pixel coordinates.
...
See prompt 'mobile-automation-best-practices'`, {
    ...
    tool("mobile_type_keys", "LOW-LEVEL TOOL: Type text into currently focused element. PREFER mobile_smart_fill_form for forms or mobile_smart_login for login (1 call vs 10-20). Use this only if: 1) Smart tools failed, 2) Element already focused, 3) Simple single-field input. See prompt 'mobile-automation-best-practices' for full guide.", {
        device: zod_1.z.string().describe("The device identifier to use. Use mobile_list_available_devices to find which devices are available to you."),
        ...
        clearField: zod_1.z.boolean().optional().describe("If true, clears the field before typing. Use this to avoid text concatenation issues (default: false).")
    ...
    // APP LOGS & DIAGNOSTICS
    // ===========================
    tool("mobile_get_app_logs", "Get application logs from the device. This shows recent log entries from a specific app, which helps with debugging. Use this when you need to see what errors or messages the app is logging.", {
        device: zod_1.z.string().describe("The device identifier to use. Use mobile_list_available_devices to find which devices are available to you."),
        packageName: zod_1.z.string().describe("The package name (Android) or bundle ID (iOS) of the app to get logs for. Use mobile_list_apps to find available package names."),
        lines: zod_1.z.number().optional().describe("Number of most recent log lines to return. Defaults to 100 if not specified.")
    }, async ({ device, packageName, lines }) => {
        const robot = getRobotFromDevice(device);
        const numLines = lines || 100;
        // Check if it's Android or iOS robot
        if (robot instanceof android_1.AndroidRobot || robot instanceof ios_1.IosRobot) {
    ...
    tool("mobile_clear_app_logs", "Clear application logs on the device. This clears all logs (not just one app) on Android devices. iOS does not support clearing logs programmatically.", {
        device: zod_1.z.string().describe("The device identifier to use. Use mobile_list_available_devices to find which devices are available to you.")
    }, async ({ device }) => {
        const robot = getRobotFromDevice(device);
        // Check if it's Android or iOS robot
    ...
    tool("mobile_get_crash_logs", "Get crash logs from the device. This shows recent app crashes with stack traces, which is critical for debugging crash issues. Use this when an app has crashed and you need to understand why.", {
        device: zod_1.z.string().describe("The device identifier to use. Use mobile_list_available_devices to find which devices are available to you."),
        packageName: zod_1.z.string().optional().describe("Optional package name (Android) or bundle ID (iOS) to filter crashes for a specific app. If not provided, returns all recent crashes."),
        lines: zod_1.z.number().optional().describe("Number of log lines to search through for crashes. Defaults to 500 if not specified.")
    }, async ({ device, packageName, lines }) => {
        const robot = getRobotFromDevice(device);
    ...
    tool("mobile_get_system_errors", "Get system error logs from the device. This shows system-level errors that might affect app behavior. Use this when you need to understand system-level issues or investigate performance problems.", {
        device: zod_1.z.string().describe("The device identifier to use. Use mobile_list_available_devices to find which devices are available to you."),
        lines: zod_1.z.number().optional().describe("Number of recent error log lines to return. Defaults to 100 if not specified.")
    }, async ({ device, lines }) => {
        const robot = getRobotFromDevice(device);
        const numLines = lines || 100;
        // Check if it's Android or iOS robot
