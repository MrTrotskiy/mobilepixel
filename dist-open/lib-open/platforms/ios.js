...
class IosRobot {
    ...
    _wdaInitialized = false; // Track if WDA was checked
    ...
    async assertTunnelRunning() {
        if (await this.isTunnelRequired()) {
            if (!(await this.isTunnelRunning())) {
                throw new robot_1.ActionableError("iOS tunnel is not running, please see https://github.com/MrTrotskiy/mobilepixel/wiki/");
            }
        }
    }
    /**
    ...
    async wda() {
        ...
        if (!(await this.isWdaForwardRunning())) {
            throw new robot_1.ActionableError("Port forwarding to WebDriverAgent is not running (tunnel okay), please see https://github.com/MrTrotskiy/mobilepixel/wiki/");
        }
        const wda = new webdriver_agent_1.WebDriverAgent("localhost", WDA_PORT);
        if (!(await wda.isRunning())) {
            throw new robot_1.ActionableError("WebDriverAgent is not running on device (tunnel okay, port forwarding okay), please see https://github.com/MrTrotskiy/mobilepixel/wiki/");
        }
     ...
     */
    getCrashLogs(bundleId, lines = 500) {
...
class IosManager {
    ...
    static goIosInstalled = null;
    static goIosWarningShown = false;
    isGoIosInstalled() {
