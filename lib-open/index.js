...
const startSseServer = async (port) => {
    ...
    app.listen(port, () => {
        (0, logger_1.error)(`MobilePixel ${(0, server_1.getAgentVersion)()} SSE server listening on http://localhost:${port}/mcp`);
    });
};
const startStdioServer = async () => {
    try {
        ...
        (0, logger_1.error)("MobilePixel server running on stdio");
    }
    catch (err) {
